<?php 
session_start();
// index.php olduğu için giriş kontrolü (header yönlendirmesi) kaldırıldı.
...
try {
    ...
    // --- BİLDİRİM SİSTEMİ (MESAJ SAYISI ÇEKME) ---
    $unread_count = 0;
    $kullanici = null; // Hata almamak için varsayılan değer
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        // Kullanıcı profil çekme işlemi (Navbar için)
        $u_sorgu = $db_conn->prepare("SELECT profil_resmi FROM users WHERE id = ?");
    ...
    $sql = "SELECT i.*, m.marka_adi as marka, mo.model_adi as model, s.sehir_adi as sehir 
            FROM ilanlar i
    ...
    // --- YENİ EKLENEN KATEGORİ (ARAÇ/EMLAK) VE DURUM (SATILIK/KİRALIK) FİLTRELERİ ---
...
<head>
    ...
    <style>
        ...
        .btn-login-noir {
            ...
            color: #0d6efd;
            border-radius: 30px; /* Yumuşatılmış kenar */
            padding: 8px 25px;
            font-weight: 600;
        ...
        .btn-register-noir:hover {
            background: #0b5ed7;
            box-shadow: 0 0 20px rgba(13, 110, 253, 0.5);
            color: #fff;
        }

        /* --- EMLAK KATEGORİSİ İÇİN EK BİLGİ RO rozetleri --- */
        .badge-category {
            position: absolute;
            top: 15px;
            left: 15px;
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav me-auto">
                <li class="nav-item me-3"><a class="nav-link" href="ilanlarim.php">İlanlarım</a></li>
                <li class="nav-item me-3"><a class="nav-link" href="favoriler.php">Favoriler</a></li>
            ...
            <div class="dropdown">
                ...
                <div class="d-flex align-items-center dropdown-toggle" data-bs-toggle="dropdown" style="cursor:pointer;">
                    <div class="user-profile-circle me-2">
                        <?php echo (!empty($kullanici) && !empty($kullanici['profil_resmi'])) ? '<img src="img/profil/'.$kullanici['profil_resmi'].'" style="width:100%;height:100%;object-fit:cover;">' : strtoupper(substr($_SESSION['user_name'] ?? 'U', 0, 1)); ?>
                    </div>
                    <span class="text-white-50 small"><?php echo $_SESSION['user_name'] ?? 'Kullanıcı'; ?></span>
                </div>
                <ul class="dropdown-menu dropdown-menu-end bg-black border-secondary text-white">
                    <li><a class="dropdown-item text-white" href="profil_duzenle.php"><i class="fas fa-user-cog me-2 text-primary"></i>Profilim</a></li>
                    <li><hr class="dropdown-divider" style="border-color: rgba(255,255,255,0.1);"></li>
                    <li><a class="dropdown-item text-danger" href="cikis.php"><i class="fas fa-sign-out-alt me-2"></i>Çıkış Yap</a></li>
                </ul>
                <?php else: ?>
                <div class="d-flex gap-2">
                    <a href="giris.php" class="btn-login-noir">Giriş Yap</a>
                    <a href="kayit.php" class="btn-register-noir">Kayıt Ol</a>
                </div>
                <?php endif; ?>
            </div>
        </div>
...
<header class="hero-section text-center">
    <div class="py-5">
        ...
        <p class="lead text-center">En seçkin araç ve premium gayrimenkul ilanlarını keşfetmeye hemen başlayın.</p>
    </div>
</header>
...
<div class="container">
    <div class="filter-card">
        <form action="index.php" method="GET" class="row g-3">
            <div class="col-md-12">
                <input type="text" name="ara" class="form-control" placeholder="Anahtar kelime veya ilan başlığı arayın..." value="<?php echo htmlspecialchars($_GET['ara'] ?? ''); ?>">
            </div>
            
            <!-- SEÇİLEN KATEGORİYE GÖRE FILTRELEME ALANLARI -->
            <div class="col-md-3">
            ...
            <!-- ARAÇLARA ÖZEL ALT FİLTRELER (Kategori Araba Seçildiyse veya Boşsa Çalışır) -->
    ...
    <div class="row mt-5 g-4">
        <h3 class="mb-2 fw-bold">Seçkin İlan Listesi</h3>

        <?php if (!empty($ilanlar)): ?>
            <?php foreach ($ilanlar as $ilan): ?>
                <div class="col-md-4">
                    <div class="card car-card shadow-sm">
                        <!-- Kategoriyi kartın üzerinde dinamik rozetle gösteriyoruz -->
                        ...
                        <img src="<?php echo !empty($ilan['ilan_resmi']) ? 'img/ilanlar/'.$ilan['ilan_resmi'] : 'https://via.placeholder.com/600x400/1e1e1e/ffffff?text=NOIR'; ?>" class="card-img-top car-img" alt="İlan Görseli" onerror="this.src='https://via.placeholder.com/600x400/1e1e1e/ffffff?text=NOIR'">
                        <div class="card-body d-flex flex-column">
                            ...
                            <p class="mb-1 small text-white-50">
                                ...
                                if(($ilan['ilan_tipi'] ?? 'Araba') == 'Emlak') {
                                    echo ($ilan['sehir'] ?? 'Şehir Yok') . " | Premium Gayrimenkul";
                                } else {
        ...
        <?php else: ?>
            <div class="col-12">
                <div class="no-results">
                    ...
                    <a href="index.php" class="btn btn-outline-primary mt-2">Tüm İlanları Gör</a>
                </div>
            </div>
        <?php endif; ?>
    </div>
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-4 mb-4">
                <h5>NOIR</h5>
                <p class="small text-white-50">Türkiye'nin en güvenilir lüks araç ve gayrimenkul platformu.</p>
            </div>
            <div class="col-md-2 mb-4">
                <h6>Hızlı Linkler</h6>
                <ul class="list-unstyled small">
                    <li><a href="ilanlarim.php" class="text-white-50">İlan Ver</a></li>
                    <li><a href="favoriler.php" class="text-white-50">Favorilerim</a></li>
                    <li><a href="mesajlarim.php" class="text-white-50">Mesajlarım</a></li>
                </ul>
            </div>
            <div class="col-md-3 mb-4">
                <h6>İletişim</h6>
                <p class="small mb-1 text-white-50"><i class="fas fa-map-marker-alt me-2 text-primary"></i> Ankara, Türkiye</p>
            </div>
            <div class="col-md-3 mb-4">
                <h6>Bizi Takip Edin</h6>
                <div class="d-flex gap-3">
                    <a href="#" class="fs-5 text-white-50"><i class="fab fa-instagram"></i></a>
                    <a href="#" class="fs-5 text-white-50"><i class="fab fa-twitter"></i></a>
                </div>
            </div>
        </div>
        <hr>
        <p class="text-center small mb-0 text-white-50">&copy; 2026 NOIR. Tüm hakları saklıdır.</p>
    </div>
</footer>
...
<script>
    ...
    window.onload = () => {
        ...
        // Sayfa yüklendiğinde kategori durumunu kontrol et
